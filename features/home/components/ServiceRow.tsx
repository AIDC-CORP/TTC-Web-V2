        ...
        {services.map((svc, idx) => (
          <article key={svc.id} className="group bg-white rounded-3xl overflow-hidden shadow-sm ring-1 ring-gray-100 hover:shadow-lg transition">
            <div className="relative">
              <img
                src={svc.image}
                alt={t(`home.services.items.${svc.id}.name`)}
            ...
            <div className="pt-10 px-6 pb-6">
              <div className="text-red-600 text-xs font-semibold tracking-widest uppercase mb-2">{t('home.services.category')}</div>
              <h3 className="text-lg font-bold mb-2 group-hover:text-blue-700 transition">{t(`home.services.items.${svc.id}.name`)}</h3>
              <p className="text-gray-600 text-sm leading-6">{t(`home.services.items.${svc.id}.summary`)}</p>
            </div>
          </article>
        ))}
      </div>
      ...
      >
        {services.map((svc, idx) => (
          <article key={svc.id} className="min-w-[85%] sm:min-w-[60%] snap-start bg-white rounded-3xl overflow-hidden shadow-sm ring-1 ring-gray-100">
            ...
            <div className="pt-10 px-6 pb-6">
              <div className="text-red-600 text-xs font-semibold tracking-widest uppercase mb-2">{t('home.services.category')}</div>
              <h3 className="text-lg font-bold mb-2">{t(`home.services.items.${svc.id}.name`)}</h3>
              <p className="text-gray-800 text-base leading-relaxed">{t(`home.services.items.${svc.id}.summary`)}</p>
            </div>
          </article>
        ))}
      </div>
